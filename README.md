...
- **단일 직렬화를 통한 1:N 채팅 병목 제거**: 다수에게 동일한 메시지를 보낼 때 발생하는 직렬화 오버헤드를 원천 차단(1회 변환 후 재사용)하여 CPU 사용률을 절감
- **TPS 극대화**: Scale-out 없이 아키텍처 재설계만으로 동일 하드웨어 기준 초당 처리량(TPS)을 **1.6 msg/s에서 24.6 msg/s**로 개선.
- **응답 속도 최적화**: Blocking I/O 스레드 고갈 문제를 해결하여 응답 시간 **99.9% 단축 (50s → 0.02s)** 및 실시간 전송 보장.
